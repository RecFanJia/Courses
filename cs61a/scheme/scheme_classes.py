...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        # Iterate over both formals and values together
        while formals is not nil and vals is not nil:
            # Each iteration binds one formal to one value in the new frame
            new_frame.define(formals.first, vals.first)
            formals = formals.rest
            vals = vals.rest

        # After the loop, both formals and vals should be nil if the lengths were equal
        if formals is not nil or vals is not nil:
                raise SchemeError('Mismatched number of formals and values after processing')
        return new_frame
            # END PROBLEM 8
