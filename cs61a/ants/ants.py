...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if self.can_contain(original_ant):
                place.ant = self
                self.ant_contained=original_ant
            elif original_ant.can_contain(self):
                original_ant.ant_contained = self
            else:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for b in list(current_place.bees):
            if self.health>0:
                Bee.reduce_health(b, amount)
            else:
                Bee.reduce_health(b,self.damage + amount)
            # END Problem 5
...
class HungryAnt(Ant):
    ...
    damage = 0
    food_cost = 4
    chewing_turns= 3# OVERRIDE CLASS ATTRIBUTES HERE
    
    implemented = True   # Change to True to view in the GUI    

    def __init__(self,health=1):
        """Create an Insect with a not chewing and a starting PLACE."""
...
class TankAnt(ContainerAnt):
    """TankAnt provides protection to other Ants and extra dmg."""
    ...
    def action(self, gamestate):
        if self.place.bees:
            for b in list(self.place.bees):
                Bee.reduce_health(b, self.damage)
